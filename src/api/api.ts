import { CART_PRODUCTS_KEY, LIKED_PRODUCTS_KEY, PRODUCT_GROUP_LIST } from './constatns';
...
class Api {
  ...
  private getItemsFromLS(key: string) {
    return JSON.parse(localStorage.getItem(key) ?? '[]') as ProductM[];
  }

  public async getProductsInfo(): Promise<ProductsInfoM> {
    await this.delay();

    const likedProducts = this.getItemsFromLS(LIKED_PRODUCTS_KEY);
    const cartProducts = this.getItemsFromLS(CART_PRODUCTS_KEY);
  ...
  public async setToCart(product: ProductM): Promise<void> {
    ...
    const cartProduct = this.getItemsFromLS(CART_PRODUCTS_KEY);
